    ...
    async function fetchData() {
      try {
        ...
        const allLocks = await LockContract.methods
          .getCumulativeNormalTokenLockInfo(0, 1000)
          .call();
        console.log(allLocks);
        let temp: any = [];
        for (let i = 0; i < allLocks.length; i++) {
          const contract = await getERC20Contract(allLocks[i].token);
          const name = await contract.methods.name().call();
          const symbol = await contract.methods.symbol().call();
          const decimals = await contract.methods.decimals().call();
          const t = { name, symbol, decimals, data: allLocks[i] };
          temp.push(t);
              ...
              return (
                ...
                >
                  <Box width="50%" display="flex" alignItems={"center"}>
                    <Box>
                      <img src="/icons/thinking.png" alt={'thinking'} width={44} />
                    </Box>
                    <Box ml="10px">
